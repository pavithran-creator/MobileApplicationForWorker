...
export async function POST(
  ...
  try {
    ...
    dbStore.auditLogs.push({
      ...
      meta: `Rejected workforce allocation for ${alloc.service_name} (${alloc.area})`,
    ...
    return NextResponse.json({
      id: aid,
      status: "REJECTED",
      message: "Workforce allocation recommendation declined"
    });
  } catch (err: any) {
    return NextResponse.json({ detail: err?.message || "Failed to decline allocation" }, { status: 500 });
  }
}
